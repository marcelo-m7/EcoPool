...
def carregar_dados_emoncms(ph, treino, inicio, fim):
    ...
    if treino == True:
        ...
        #necessario solicitar dados do futuro do meteo e converter para uma sequencia de 30 minutos
       ...
       # y_id = np.array([previsto_array[:, 1]]).reshape(tamanho, 1)
        y_id = np.array([previsto_array[:, 1]]).reshape(20, 1)
        y_val = np.array([tpool_array[-fim_treino - 15:, 1]]).reshape(ph + 15, 1)
...
if __name__ == "__main__":
    ...
    if train == True:
            ...
            # lista_inicio= registro_var['Inicio']
            # lista_cenarios = registro_var['Cenario']
            # lista_duracao = registro_var['Duration']
            ...
            for cont in range(0,4):
                #substituir inicio e fim pela leitura direta de cada linha do ficheiro
                    ...
                    # inicio = lista_inicio[cont]
                    # fim = inicio + lista_duracao[cont]
                    ph = 12
        ...
        step = 0; ph= 4;
        inicio = calendar.timegm(time.strptime('2023-06-13 00:00:00', '%Y-%m-%d %H:%M:%S'))
        fim = calendar.timegm(time.strptime('2023-06-15 15:00:00', '%Y-%m-%d %H:%M:%S'))
